...
## Contents
- `prompt-log-append.sh` — the `UserPromptSubmit` hook behind `files/prompt_log/` (wired in `.claude/settings.local.json`). Appends each prompt in the `--- / timestamp / text` format to `prompt-log-YYYY-MM.md`, rolling over monthly. Typing **`pinthat`** as the whole prompt appends Claude's previous response instead, prefixed `[Claude — response to the prompt above]` so AI text in the log stays identifiable, then exits 2 — which erases the prompt and blocks it, so a pin costs no model turn at all. Bash + jq, no venv. Change the output path or trigger word at the top of the script.
...
- `build-council-index.py` / `verify-council-index.py` — build and verify
  ...
  document: the date the city printed in the masthead, the date in Brandon's filename,
...
- `parse-assessment.py` / `verify-assessments.py` — build and verify
  ...
  directory from their own location, so they run from anywhere. Moved here 2026-07-29 from
  a local `tn_property_assessments/bin/`.
...
- `verify-990.py` — four independent checks against the Gallatin IDB 990 filings in
  ...
  that directory.
- `build-hamilton-master.py` — builds `state_of_tennessee/tn_comptroller_pilot_reports/derived/hamilton-county-pilot-master-2014-2025.csv` from the yearly Hamilton County reports.
