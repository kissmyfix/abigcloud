...
| `files/bin/sync-voice-notes.py` | Re-derives every status marker in `memory/brandon-voice-notes.md` by checking whether that note's comment still survives under `web/content/`. The status is a derived field and is never typed by hand; `ship.sh` runs this first on every publish. Two records of one fact drift, and this one did — eleven notes sat marked OPEN long after being closed. |
| `files/bin/ship.sh` | **Publishing, in one command.** Build, commit, push, wait on the Action, purge Cloudflare's cache, confirm the live pages serve 200. Credentials for the purge live at `~/.config/abigcloud/cf-purge`, outside the repo; a missing file skips the purge rather than failing the deploy. On a failed deploy it prints the failing step and the log lines that explain it, via `gh`. Use it instead of running the steps by hand. |
| `inbox/` | **This project's Downloads folder.** Anything dropped here is unfiled and unvetted; gitignored, and normally empty. `files/bin/intake-watch.sh` watches it and runs `files/bin/intake.py` on anything that lands: hash, duplicate check against the whole archive, format profile, OCR when there is no text layer, extraction, and a manifest row. It never files anything itself. What needs a decision goes to `inbox/NEEDS-FILING.md`. Filing conventions: `.claude/skills/extract-pdf-source/SKILL.md` under Intake. |
| `files/MANIFEST.csv` | The intake ledger, one row per document that came through `inbox/`: retrieval date, sha256, format, trust bucket, where it was filed, source URL. Gitignored. **Series completeness is a query against this, not an act of memory.** |
| `files/bin/` | Every project script — PDF extraction and profiling, whisper transcription, Comptroller CSV builders and verifiers, the parcel-assessment parser. Run with `files/venv/bin/python`. **`990-master.py`** aggregates a directory of IRS 990 XML filings into one master XML plus an HTML render, carrying both the pre-2016 and post-2016 element names for every field and leaving a genuinely absent value blank rather than zero. **`inbox-index.py`** indexes a *subject* inbox such as `sumner_county/600_small_street/inbox/` into an `INDEX.csv`, recomputing everything except `status` and `notes`, which are carried forward so a re-run never loses a note. **Do not confuse it with `intake.py`**, which serves the project-root `inbox/` only: `intake.py` is the watched staging pipeline that hashes, dedupes, OCRs and writes `files/MANIFEST.csv`, and its inbox is meant to be empty; `inbox-index.py` describes a standing pile of subject documents that is meant to persist. |
...
| `sumner_county/805_teal_drive/` | **The building at the centre of the Children Are People story**, parcel 125E/A/010.00. Bought by Diane and David Black for $1.2M on 2021-09-20, conveyed to CAP for $0 four weeks before the gala that announced it, sold for $1.2M on 2026-01-12 to Keiths Canines Holdings LLC, five months before the press reported a "$1.5M legacy endowment" going to Shalom Zone. Its `README.md` carries the full chain of title, the dissolution sequence, and the SOS record for the buyer. **Read that README before opening anything in here.** |
...
| `state_of_tennessee/tn_property_assessments/` | Parcel records from the state viewer. **`gallatin_industrial_park/`** holds the 29-parcel collection of maps 105/106/111/112 taken 2026-08-29, with a `derived/parcels.csv` dataset; **`classification_index/`** holds thirteen CSV exports of the state search, one per classification code, covering all of Sumner County — a finding aid with parcel IDs and owners but no figures. Both have their own README. The assessor's owner field carries at least seven spellings of the Gallatin IDB, so **never search these by owner name**; search by classification or by control map. |
